...
5. **Fase E — Polimento UI/UX aplicado** ✅ concluída em 2026-07-24: confirmação em dois cliques para resetar senha/inativar usuário, filtro por transportadora + paginação nos logs, validação de range de data invertido no histórico, contraste de borda/foco corrigido, loading state no upload.
6. **Fase F — Testes** 🟡 parcial, decisão registrada em 2026-07-24: os 5 testes unitários do parser estão feitos (`lib/xlsx-parse.test.ts`, cobrindo arquivo válido, transportadora errada, aba faltando, célula inválida, e a garantia de que o parser não valida soma sozinho). O teste de ponta a ponta via Playwright foi feito repetidas vezes manualmente nesta sessão (download, upload como rascunho, upload como envio final, trava de dia já enviado), mas **não foi formalizado como suíte permanente** — decisão consciente de não montar `playwright.config.ts`/pasta de E2E agora, já que o projeto não tem CI configurado e um E2E sem pipeline automático tem valor limitado. Revisitar se/quando houver CI.
